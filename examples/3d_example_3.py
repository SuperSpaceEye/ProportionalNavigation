...
while True:
    ...
    pitch_ar_change = np.sin(pitch_gimbal) * pursuer_v * dt
    yaw_ar_change   = np.sin(yaw_gimbal)   * pursuer_v * dt

    lagged_pursuer.pitch += pitch_ar + pitch_ar_change
    lagged_pursuer.yaw   += yaw_ar   + yaw_ar_change
...
print("pursuer_diff", abs(sum(actual - lagged)) / len(actual))

actual = np.array(log["target"])
lagged = np.array(log["lagged_target"])

print("target_diff", abs(sum(actual - lagged)) / len(actual))
