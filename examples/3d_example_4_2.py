...
while True:
    ...
    ret = pn.ai_true_3d(lagged_pursuer_phys, lagged_target, N)
    R = ret["R"]
    ...
    psi_cmd, theta_cmd = ret["psi"], ret["theta"]

    v_psi = pursuer_acc * np.array([np.cos(psi_cmd), np.sin(psi_cmd), 0])
    v_theta = pursuer_acc * np.array(
        [np.sin(theta_cmd) * np.cos(psi_cmd), np.sin(theta_cmd) * np.sin(psi_cmd), np.cos(theta_cmd)])

    a_cmd = (v_psi + v_theta - pursuer_physic.vel)

    new_yaw, new_pitch = get_angles(a_cmd)
    ...
    actual_pursuer_acc = np.clip(np.sqrt(a_cmd.dot(a_cmd)), 0, pursuer_acc)
